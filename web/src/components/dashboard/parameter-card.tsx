...
export function ParameterCard({ title, value, unit, status, range, trend, icon: Icon, color }: ParameterCardProps) {
  ...
  return (
    <div
      className="relative p-4 rounded-xl transition-all duration-200 hover:-translate-y-px cursor-default border dark:bg-slate-900/40 bg-white shadow-sm light:border-slate-200 dark:border-white/10"
      style={{ 
        boxShadow: status !== 'normal' ? `0 0 18px ${cfg.shadow}` : undefined
      }}
    >
      {/* Header */}
      ...
      <div className="flex items-end justify-between mb-3">
        <div className="flex items-baseline gap-0.5">
          <span className="text-[26px] font-bold font-mono dark:text-white text-slate-900 leading-none" style={{ textShadow: `0 0 12px ${color}35` }}>
            {value.toFixed(unit === "" ? 2 : 1)}
          </span>
          <span className="text-xs dark:text-gray-500 text-slate-400 font-mono pb-0.5">{unit}</span>
        </div>
      ...
      <div className="space-y-1">
        <div className="relative h-[3px] rounded-full dark:bg-[#1e3a5f] bg-slate-100">
          <div
          ...
          <div
            ...
            style={{ left: `calc(${pct * 100}% - 3.5px)`, backgroundColor: color, border: "2px solid var(--background)", boxShadow: `0 0 5px ${color}` }}
          />
        </div>
        <div className="flex justify-between items-center">
          <span className="text-[8px] font-mono dark:text-gray-700 text-slate-400">{range.min}{unit}</span>
          <span className={cn("text-[10px] font-mono font-semibold", trend > 0 ? "text-emerald-500" : trend < 0 ? "text-rose-500" : "dark:text-gray-500 text-slate-400")}>
